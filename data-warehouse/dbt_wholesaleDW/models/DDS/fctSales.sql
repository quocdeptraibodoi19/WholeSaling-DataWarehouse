...
with source_data as (
    ...
    from {{ ref('sales_SalesOrderHeader') }} sales_SalesOrderHeader
    inner join {{ ref('sales_SalesOrderDetail') }} sales_SalesOrderDetail
...
{% if is_incremental() %}
    where fct_updated_at >= (select max(fct_updated_at) from {{ this }})
{% endif %}
