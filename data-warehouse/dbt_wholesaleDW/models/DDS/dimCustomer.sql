...
with source_data as (
    ...
    from {{ ref('sales_Customer') }} sales_Customer
    left join {{ ref('person_Person') }} person_Person
        on sales_Customer.person_id = person_Person.bussiness_entity_id
    left join {{ ref('person_BusinessEntityAddress') }} person_BusinessEntityAddress
        on person_BusinessEntityAddress.bussiness_entity_id = person_Person.bussiness_entity_id
    left join {{ ref('person_Address') }} person_Address
        on person_Address.addressid = person_BusinessEntityAddress.address_id
    left join {{ ref('person_AddressType') }} person_AddressType
