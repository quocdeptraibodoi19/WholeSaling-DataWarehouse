...
from ingest.Product_System.delta_load import (
    update_delta_keys,
    delta_HR_to_HDFS,
...
with DAG(
    ...
    description=f"A delta load data pipleline to ingest data from {ConstantsProvider.get_Product_source()}",
    schedule=timedelta(hours=3),
    start_date=datetime(2024, 1, 1),
    catchup=False,
) as dag:
    product_task_generator = DeltaLoadTaskGenerator(
        dag=dag,
